...
JarInflater ji=new JarInflater(((PhysicalFile)Editor.PhysicalFile.PhysicalFiles.get("compiled.zip")).getContent());

System.out.println("classloader classname "+classname);

			DataInputStream di=new DataInputStream(ji.getInputStream(classname));
			int cou=0;
			...
			di=new DataInputStream(ji.getInputStream(classname));
			di.read(by);
